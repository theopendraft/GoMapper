    ...
    if (routePins.length >= 2) {
      ...
      routingControl.on("routesfound", (e: any) => {
        ...
        if (routes && routes.length > 0) {
          const primaryRoute = routes[0]; // Use the primary route

          // Guard against undefined instructions or summary
          if (primaryRoute.instructions && primaryRoute.summary) {
            const instructions: Instruction[] = primaryRoute.instructions.map(
              (instr) => ({
                text: instr.text || "Unnamed step", // Provide a fallback for undefined text
